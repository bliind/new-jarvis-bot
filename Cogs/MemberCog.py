...
def check_member_age(member: discord.Member, new_account_days: int):
    now = datetime.datetime.now(datetime.timezone.utc)
    diff = now - member.created_at

    return diff.days >= new_account_days

def check_server_age(member: discord.Member, member_hours: int):
    now = datetime.datetime.now(datetime.timezone.utc)
    diff = now - member.joined_at

    return (diff.total_seconds() / 3600) >= member_hours
...
class MemberCog(commands.Cog):
            ...
            try:
                ...
                if role:
                    ...
                    for member in members:
                        if check_member_age(member, configs.new_account_days):
        ...
        for server in self.bot.guilds:
            ...
            try:
                ...
                if role:
                    ...
                    for member in members:
                        if check_server_age(member, configs.member_hours) \
    ...
    async def check_new_member_age(self, member: discord.Member):
        ...
        if not check_member_age(member, configs.new_account_days):
